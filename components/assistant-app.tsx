...
import { GenerateRequest, RequestOptions, StructuredResponse, UserProfile } from "@/lib/types";
...
export function AssistantApp() {
  ...
  async function submitRequest() {
    const payload: GenerateRequest = { profile, task, refinement, options, history };
  ...
  return (
    <main className="mx-auto max-w-6xl p-6">
      ...
      <p className="mt-1 text-sm text-slate-600">Capture profile → submit request → generate structured report → refine iteratively.</p>
      ...
      <section className="mt-6 grid gap-6 md:grid-cols-2">
        <div className="rounded-xl bg-white p-4 shadow">
          ...
          <select
            ...
            onChange={(e) => setProfile({ ...profile, preferredFormat: e.target.value as UserProfile["preferredFormat"] })}
          >
        ...
        <div className="rounded-xl bg-white p-4 shadow">
          ...
          <div className="mt-4 flex gap-2">
            <button disabled={!canSubmit} onClick={submitRequest} className="rounded bg-slate-900 px-4 py-2 text-white disabled:opacity-50">
              Generate / Regenerate
            </button>
            <button
              onClick={() => output && setSavedReports((prev) => [output, ...prev])}
      ...
      <section className="mt-6 rounded-xl bg-white p-4 shadow">
        ...
        {!output ? (
          ...
          <div className="mt-3 space-y-3 text-sm">
            ...
            {output.citations && <ReportBlock title="Citations" lines={output.citations} />}
            <details>
      ...
      <section className="mt-6 rounded-xl bg-white p-4 shadow">
        ...
        <p className="text-sm text-slate-600">Refinement log: {history.length ? history.join(" | ") : "none"}</p>
        <p className="mt-1 text-sm text-slate-600">Saved revisions: {savedReports.length}</p>
...
function SelectField({
  ...
  return (
    <div>
      ...
      <select className="mt-1 w-full rounded border p-2 text-sm" value={value} onChange={(e) => onChange(e.target.value)}>
        {options.map((option) => (
